...
onready var store_locally: Button = $HBoxContainer/HBoxContainer/Button
...
func _on_ignorables_browse() -> void:
	var popup: FileDialog = load(BROWSE_IGNORABLES_POPUP_PATH).instance()
	popup.current_dir = text
...
func _on_ignorables_confirm() -> void:
	var ignorables := _parse_ignorables(text_edit, text)
